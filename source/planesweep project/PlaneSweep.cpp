...
bool PlaneSweep::hasRelation(Seg2D) {
    /*
     * if there is intersection or overlap,
     * call splitLines
     * return true.
     */
    return false;
...
void PlaneSweep::addLeft(Seg2D &seg2D) {
    if (!hasRelation(seg2D)) {
        PlaneSweepLineStatusObject sweepLineStatusObject(seg2D);
        sweepLineStatus->insert(sweepLineStatusObject);
    }
}
...
{
    PlaneSweepLineStatusObject sweepLineStatusObject(seg2D);
    sweepLineStatus->deleteKey(sweepLineStatusObject);
}
