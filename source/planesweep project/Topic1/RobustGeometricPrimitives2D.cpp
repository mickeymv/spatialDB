...
bool HalfSeg2D::operator<(const HalfSeg2D &operand) {
    ...
    if (dominatingPointOfSeg1 < dominatingPointOfSeg2
        ...
        || (this->isLeft == operand.isLeft && SegmentIsCollinearAndMeetsLeftEndpoint(seg1, seg2) &&
            SegmentIsLesserThanSegment(seg1, seg2))
            ) {
        return true;
    }
    else {
...
bool PointLiesOnSegmentAndNotEndpoints(Poi2D &q, Seg2D &seg) {
    Number minp1p2x;
    Number maxp1p2x;
    Number minp1p2y;
    Number maxp1p2y;
...
bool SegmentLiesOnSegment(Seg2D &seg1, Seg2D &seg2) {
    ...
    if ((PointLiesOnSegment(seg1.p1, seg2) && (PointLiesOnSegment(seg1.p2, seg2))) ||
        (PointIsCollinearToSegment(seg1.p1, seg1)) && (PointLiesOnSegment(seg1.p2, seg2)) ||
        (PointIsCollinearToSegment(seg1.p2, seg1)) && (PointLiesOnSegment(seg1.p1, seg2)) ||
        (PointLiesOnSegment(seg2.p1, seg1) && (PointLiesOnSegment(seg2.p2, seg1))))
        return true;
    else
        return false;
}
...
bool Intersects(Seg2D &seg1, Seg2D &seg2) {
    ...
    if (PointLiesOnSegment(seg1.p1, seg2) || PointLiesOnSegment(seg1.p2, seg2) || PointLiesOnSegment(seg2.p1, seg1) ||
        PointLiesOnSegment(seg2.p2, seg1))
        return true;
    // Special Cases
...
bool Touch(Seg2D &seg1, Seg2D &seg2) {
    if ((!SegmentIsCollinear(seg1, seg2)) &&
        (PointLiesOnSegmentAndNotEndpoints(seg1.p1, seg2) || PointLiesOnSegmentAndNotEndpoints(seg1.p2, seg2)))
        return true;
    else
        return false;
}
            ...
            if (forI == 1 && i == (totalNumberOfVertices - 1)) {
                ...
                if (intersects) {
                    ...
                    if (PointLiesOnSegmentAndNotEndpoints(polygon.vertices[i], segC) ||
                        PointLiesOnSegmentAndNotEndpoints(polygon.vertices[k], segC)) {
                    ...
                    if (pointCheck) {
                        ...
                        bool pointsOntheSameSideOfSeg = PointLiesOnSameSideOfSegment(polygon.vertices[j],
                                                                                     polygon.vertices[i + 1], segR);
