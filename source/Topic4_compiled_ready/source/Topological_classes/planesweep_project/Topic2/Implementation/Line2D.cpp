...
Line2D::Line2D(const Line2D &source) {
	...
	std::copy(source.handle->mapHseg.begin(), source.handle->mapHseg.end(),
			  std::inserter(handle->mapHseg, handle->mapHseg.end()));
}
...
Line2D::Line2D(Line2D &&source) {
	...
	std::move(source.handle->mapHseg.begin(), source.handle->mapHseg.end(),
			  std::inserter(handle->mapHseg, handle->mapHseg.end()));
}
...
bool Line2D::operator<(Line2D &operand) {
	if (isEmptyLine2D() || operand.isEmptyLine2D())
		return false;
	int x = (handle->segments.size() < operand.handle->segments.size()) ? handle->segments.size()
																		: operand.handle->segments.size();
...
bool Line2D::ConstBlockIterator::operator==(const ConstBlockIterator &rhs) const {
	return ((this->handlei->current == rhs.handlei->current) &&
...
std::ostream &operator<<(std::ostream &os, const Line2D::ConstBlockIterator &output) {
	...
	for (int x = 0; x < output.handlei->current->mapHseg.find(output.handlei->iteratorIndex)->second.size(); x++) {
		os << "segments" << *output.handlei->current->mapHseg.find(output.handlei->iteratorIndex)->second.at(x) <<
		" " << endl;
	}
	return os;
}
