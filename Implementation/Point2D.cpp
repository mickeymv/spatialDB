...
#include "Point2D.h"
  ...
  Point2D::Point2D(std::string textualPointList)
  {
       ...
       else if (std::regex_match( textualPointList, std::regex( " (\\(\\([0-9]+,[0-9]+\\)\\))+ " ) ))//means the string is point representation
       {
	      points->vecPoints.clear();
	      ...
	      if(count==1){points->vecPoints.clear(); return;
  ...
  bool Point2D::operator == (Point2D& operand)
  {
         ...
         for (unsigned i=0; i < points->vecPoints.size(); i++) 
	 {
           ...
           {
             if(points->vecPoints.at(i) == operand.points->vecPoints.at(j)){x=1; break;}
           }
           if(x == 0) 
             return false;
        }
