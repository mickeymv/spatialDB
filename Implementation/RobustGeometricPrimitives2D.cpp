...
std::istream&operator>>(std::istream& is, const AttrHalfSeg2D& input)
{
			...
			if(forI==1 && i==(totalNumberOfVertices - 1)){
				...
				if(intersects){
					...
					if(pointCheck){
						...
						bool pointsOntheSameSideOfSeg = PointLiesOnSameSideOfSegment(polygon.vertices[j],polygon.vertices[i+1],segR);
