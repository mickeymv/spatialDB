...
void generateOnSquare(int number, double max, char* filename) {
  ...
  myfile.open(filename, ios::out | ios::binary);
  for (int i = 0; i < number; i++) {
    if (rand() % 2) {  // generates points along the top and bottom of the
                       // square
      if (rand() % 2) {
        point.x = ((double)rand() / RAND_MAX) * max;
        point.y = max;
...
gen [filename] [int n] [double r] <s> <o>
        ...
        o=(optional: designates points as: e=exactly on shape edge, c=close to
shape edge)
*/
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 4)
    generateInSquare(atoi(argv[2]), atof(argv[3]), argv[1]);
           ...
           5) {  // in case o is not provided points generated inside the shape
    if (string(argv[4]) == "-s")
      generateInSquare(atoi(argv[2]), atof(argv[3]), argv[1]);
    if (string(argv[4]) == "-c")
      generateInCircle(atoi(argv[2]), atof(argv[3]), argv[1]);
  } else if (argc == 6) {  // in case o is provided points are generated either
                           // on the shape's edge or close to the shape's edge
    if (string(argv[5]) == "-c") {
      if (string(argv[4]) == "-s")
